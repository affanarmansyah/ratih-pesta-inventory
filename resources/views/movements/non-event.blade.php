...
@section('content')
    ...
    <p class="text-muted">Transaksi yang gak terkait pemesanan customer — misalnya narik barang dari luar, penyesuaian stok manual, dll.</p>
                ...
                <tbody>
                    @forelse($movements as $m)
                        <tr class="{{ $m->voided_at ? 'text-muted' : '' }}">
                            ...
                            <td>
                                @if(!$m->voided_at)
                                    <form action="{{ route('movements.void', $m) }}" method="POST" onsubmit="return confirm('Yakin batalkan transaksi ini? Stok akan disesuaikan kembali.')">
                                        @csrf
                    ...
                    @empty
                        <tr><td colspan="9" class="text-center">Belum ada transaksi non-event</td></tr>
                    @endforelse
                </tbody>
            </table>
            {{ $movements->links() }}
